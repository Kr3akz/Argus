...
$('btn-overlay').onclick = async () => applyOverlayView(await window.api.toggleOverlay());
...
/* ============================================================================
   ...
   Ein eigener Bildschirmzustand, kein verkleinertes Dashboard. Ueber einem
   laufenden Spiel zaehlt nur, was sich in den naechsten Minuten entscheidet:
   Zyklen, passende Risse, offene Ziele. Alles andere bleibt im Fenstermodus,
   der fuer den zweiten Monitor gedacht ist.

   Die Restzeiten laufen hier lokal weiter - ein Tick pro Sekunde aus dem
   expiry-Zeitstempel, statt auf den naechsten Abruf zu warten. Im Fenster
   faellt ein Timer, der in 30-Sekunden-Spruengen springt, kaum auf; ueber
   einem Spiel, in dem man auf die letzte Minute schaut, schon.
   ========================================================================= */

let overlayOn = false;
let overlayClickThrough = false;
let overlayTickTimer = null;
let overlayPollTimer = null;
let overlayHoverSent = null;

const OV_SOON_MS   = 5 * 60 * 1000;    // ab hier faerbt sich die Restzeit
const OV_POLL_MS   = 60000;            // Weltzustand nachladen, auch ohne Tab
const OV_MAX_FISS  = 8;
const OV_MAX_GOALS = 4;
...
function tickOverlayClocks() {
  document.querySelectorAll('#overlay-view [data-until]').forEach(el => {
    const ms = msUntil(el.dataset.until);
    el.textContent = fmtCountdown(ms);
    el.classList.toggle('soon', ms !== null && ms > 0 && ms < OV_SOON_MS);
...
function renderOverlay() {
  ...
  renderOverlayHead();
  renderOverlayCycles();
  renderOverlayFissures();
  renderOverlayGoals();
  renderOverlayFoot();
  tickOverlayClocks();
}

function renderOverlayHead() {
  const p = state && state.player;
  $('ov-mr').textContent = p ? `MR ${p.mr}` : 'MR –';
}

function renderOverlayCycles() {
  const d = worldStateCache;
  const box = $('ov-cycles');
  if (!d) {
  ...
  const c = d.cetus || {}, v = d.vallis || {}, cb = d.cambion || {};
  const rows = [
 ...
 */
function renderOverlayFissures() {
  const all = (worldStateCache && worldStateCache.fissures) || [];
  /* matches() erwartet die GANZEN Einstellungen, nicht den fissures-Teil:
     es prueft zuerst die beiden Hauptschalter und greift dann selbst auf
     settings.fissures zu. Mit dem Unterobjekt waere jeder Riss ein Nichttreffer. */
  const isHit = f => !!notificationSettings && FissureFilter.matches(f, notificationSettings);
  ...
  box.innerHTML = sorted.slice(0, OV_MAX_FISS).map(f => `
...
function renderOverlayGoals() {
  const sec = $('ov-goals-sec');
  const open = ((state && state.goals) || []).filter(g => !g.done);
  ...
  $('ov-goals').innerHTML = open.slice(0, OV_MAX_GOALS).map(g => `
    <div class="ov-row">
...
function renderOverlayFoot() {
  const ts = worldStateCache && worldStateCache.fetchedAt
    ? new Date(worldStateCache.fetchedAt).getTime() : null;
  $('ov-age').textContent = ts ? `Stand ${relativeAge(ts)}` : 'Kein Weltzustand';
}

/* ---------------- Modus schalten ---------------- */

async function applyOverlayView(st) {
  const on = !!(st && st.overlay);
  overlayOn = on;
  overlayClickThrough = !!(st && st.clickThrough);

  document.body.classList.toggle('overlay-mode', on);
  $('overlay-view').classList.toggle('hidden', !on);
  $('overlay-badge').classList.toggle('hidden', !on);
  updateClickThroughButton();

  const slider = $('ov-opacity');
  if (slider && st && Number.isFinite(st.opacity)) slider.value = String(Math.round(st.opacity * 100));

  clearInterval(overlayTickTimer); overlayTickTimer = null;
  clearInterval(overlayPollTimer); overlayPollTimer = null;
  overlayHoverSent = null;

  if (!on) return;

  if (!worldStateCache) await loadWorldState(false);
  renderOverlay();

  overlayTickTimer = setInterval(tickOverlayClocks, 1000);
  /* Im Overlay laeuft der Weltzustand weiter, auch ohne offenen Tab - genau
     dafuer ist der Modus da. */
  overlayPollTimer = setInterval(async () => {
    await loadWorldState(false);
    renderOverlay();
  }, OV_POLL_MS);
}
...
function updateClickThroughButton() {
  const btn = $('ov-click');
  if (!btn) return;
  btn.classList.toggle('on', overlayClickThrough);
  btn.title = overlayClickThrough
...
$('ov-exit').onclick = async () => applyOverlayView(await window.api.toggleOverlay());

$('ov-refresh').onclick = async () => {
  await loadWorldState(true);
  renderOverlay();
};

$('ov-click').onclick = async () => {
  const st = await window.api.setClickThrough(!overlayClickThrough);
  overlayClickThrough = !!st.clickThrough;
  updateClickThroughButton();
};
...
window.addEventListener('mousemove', e => {
  if (!overlayOn || !overlayClickThrough) return;
  const bar = $('ov-bar').getBoundingClientRect();
  const overBar = e.clientY <= bar.bottom;
  if (overBar === overlayHoverSent) return;
  overlayHoverSent = overBar;
  window.api.setOverlayHover(overBar);
});

/* Zustandswechsel kommen auch vom Hotkey, nicht nur von den Schaltflaechen. */
window.api.onOverlayChanged(st => applyOverlayView(st));
window.api.overlayState().then(st => { if (st && st.overlay) applyOverlayView(st); }).catch(() => {});
